<#
  .SYNOPSIS
      TeamViewer - Install Full & Assign
  .DESCRIPTION
      Installs TeamViewer Full and Assigns device to Company
  .NOTES
      Modify the $tvAssignmentID, $tvCurrVersion, and $tvURI variables as needed prior to use
#>
...
Start-Process MSIEXEC.EXE -WorkingDirectory 'C:\SW\TeamViewer\Full'-Wait -ArgumentList "/i TeamViewer_Full.msi /qn"
...
if (Test-Path "C:\Program Files\TeamViewer\TeamViewer.exe"){
  Write-Output "TeamViewer Full x64 detected. Running assignment command..."
  assignTV "C:\Program Files\TeamViewer\TeamViewer.exe"
  Write-Output "TeamViewer Full assignment command completed."
...
elseif (Test-Path "C:\Program Files (x86)\TeamViewer\TeamViewer.exe"){
  Write-Output "TeamViewer Full x84 detected. Running assignment command..."
  ...
  Write-Output "TeamViewer Full failed to install..."
  exit 1
}